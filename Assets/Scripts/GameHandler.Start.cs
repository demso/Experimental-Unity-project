    ...
    private void InitTilemap()
    {
        ...
        for (int y = 0; y > -tilemap.size.y; y--)
        {
            for (int x = 0; x < tilemap.size.x; x++)
            {
                ...
                string bodytype = properties.Find(property => property.m_Name.Equals("body type"))?.m_Value;
                string tileName = properties.Find(property => property.m_Name.Equals("name"))?.m_Value;
